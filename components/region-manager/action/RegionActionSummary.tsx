...
function CountCard({ icon: Icon, label, value, tone }: { icon: typeof AlertOctagon; label: string; value: number; tone: StatusLevel }) {
  const animated = useCountUp(value);
  return (
    <Card className="flex items-center gap-3.5">
      <span aria-hidden className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-control", TONE_CHIP[tone])}>
        <Icon size={19} strokeWidth={2} />
      </span>
      <div className="flex flex-col">
        <span className="font-mono text-[24px] font-semibold leading-none text-fg">{Math.round(animated)}</span>
