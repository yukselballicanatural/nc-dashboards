...
import { AGENT_PROFILE, AGENT_RANK, MOCK_DATE_LABEL } from "@/lib/mock/mock-data";
...
/**
 * Dashboard hero bandı — gösterişli imza öğe.
 * Sabit koyu gradient zemin (her iki temada beyaz metinle AA), sol tarafta
 * agent kimliği (avatar + isim + rol/takım/lokasyon/vardiya çipleri),
 * sağ tarafta cam efektli 3 öne-çıkan istatistik.
 */
...
function HeroTile({
  ...
  return (
    <div className="hero-tile flex min-w-[104px] flex-col gap-1 rounded-card px-4 py-3">
      <span className="font-body text-[11px] font-medium text-white/70">
        {label}
      </span>
...
/**
 * Gradient zemin inline verilir — CSS sınıfı yüklenmese/geçersiz kılınsa bile
 * koyu zemin garanti (açık temada beyaz metnin "patlaması" bu yüzden olmaz).
 */
  ...
  return (
    ...
    >
      <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-center">
        ...
        {/* Sağ: öne çıkan istatistikler */}
        <div className="flex flex-wrap gap-3">
          <HeroTile label={t("Cevaplanma Oranı", "Answer Rate")} value={data.answerRatePct} suffix="%" />
          <HeroTile label={t("Hedef Gerçekleşme", "Target Progress")} value={data.targetPct} suffix="%" />
          <HeroTile label={t("Takım Sırası", "Team Rank")} value={AGENT_RANK.position} suffix="#" />
          <HeroTile label={t("Genel Başarı", "Overall Score")} value={AGENT_RANK.score} />
        </div>
      </div>
    </section>
  );
