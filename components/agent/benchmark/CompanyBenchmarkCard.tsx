...
function MetricRow({ benchmark }: { benchmark: MetricBenchmark }) {
  ...
  const colors = METRIC_COLORS[benchmark.metric];
    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="font-display text-[13px] font-semibold text-fg">
          {metricLabel(benchmark.metric, lang)}
        </span>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <span className="w-8 shrink-0 text-right font-mono text-[12px] font-bold text-fg">
          {formatNumber(benchmark.agentValue)}
        </span>
      </div>
      <div className="flex items-center gap-2.5">
        ...
        <span className="w-8 shrink-0 text-right font-mono text-[11px] text-fg-secondary">
          {formatNumber(benchmark.teamAverage, 1)}
        </span>
      </div>
      <div className="flex items-center gap-2.5">
        ...
        <span className="w-8 shrink-0 text-right font-mono text-[11px] text-fg-muted">
          {formatNumber(benchmark.companyAverage, 1)}
        </span>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      >
        <SectionTitle
          hint={t(
            `Offer, Deal ve Paid sayın; takım ortalaman ve ${formatNumber(COMPANY_AGENT_COUNT)} danışmanlık şirket ortalamasıyla kıyaslanıyor.`,
            `Your Offer, Deal and Paid counts, compared against your team average and the company-wide average across ${formatNumber(COMPANY_AGENT_COUNT)} agents.`,
          )}
        >
